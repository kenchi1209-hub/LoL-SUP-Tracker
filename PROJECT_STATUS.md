...
- 試合詳細はBasic / Combat / Economy / Lane Difference / Vision / Support & Sustain / Fight & Objective / Progressionを、Match-V5・Timeline・combat timelineで確認できる値だけで表示（Fight W-E-L / 勝率、Progressionの10分・15分値と10→15分差分を含む）
...
Match Historyの詳細UIを、試合概要の簡易10人比較と、味方／敵10人の詳細Statsマトリクス・コピー機能へ再編した。ProgressionにはTimeline participantFrames由来の10分・15分Gold / XP / Level / CS / Jungle CSと10→15分差分を表示し、15分未満は推測せず欠損表示とする。Timeline由来の公開可能なStatsは、実データcoverageを確認した上でのみ追加する。
...
- Queue INの再検証は、LCU `current-summoner`のRiot ID（`gameName`／`tagLine`）を設定済み対象Riot IDと照合済みのidentityだけを同一接続内でメモリ保持する。LCU local PUUID（36文字）とRiot Account／Match-V5 PUUID（78文字）は別namespaceのため直接比較しない。gameflow遷移でcurrent-summoner endpointが一時的に失敗しても、その確認済みidentityを使えるが、endpoint復帰時の不一致またはLCU再接続時は採用を停止・再照合する。
...
- Queue IN recheckの本人確認失敗は、identifier実値を出力せず`LCU endpoint`、`empty identity`、`account mismatch`に診断分類する。分類はログ専用で、照合・採用・retry・capture・publishの安全条件を変更しない。
