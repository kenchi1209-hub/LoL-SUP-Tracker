...
def _history_match(record, rows_by_id):
    ...
    metadata.update({
        ...
        "observed_lp_delta": record.get("observed_lp_delta"),
        "lp_correction": record.get("lp_correction"),
...
def build_lp_payload(rows, version):
    ...
    for item in exact_matches:
        ...
        points.append({
            ...
            "observed_lp_delta": item.get("observed_lp_delta"),
            "lp_correction": item.get("lp_correction"),
            "lp_delta_source": item.get("lp_delta_source", "post_match_snapshot"),
